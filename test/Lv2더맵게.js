//https://school.programmers.co.kr/learn/courses/30/lessons/42626 더맵게

// 1차답안 시간초과 
	function solution(scoville, K) {
		let count = 0;
	  scoville.sort((a,b)=>a-b);
		while(scoville.length>1 && K > scoville[0] ){
			
	    let one = scoville.shift();
			let two = scoville.shift();
	    let mix = one+2*two; 
			count ++ ;
			scoville.push(mix);
	    scoville.sort((a,b)=>a-b);
	...
	function solution(scoville, K) {
		...
		while(minHeap.size() >=2 && minHeap.items[0] <K ){
			let one = minHeap.poll();
			let two = minHeap.poll();
			let mix = one + two * 2;
			count++;
			minHeap.add(mix);
		}
